...
Current status is final directed remediation committed locally through verified
implementation SHA `c688336`, pending the user-run Zotero 9.0.6 production
adapter/store smoke. The branch is not pushed; exact-SHA GitHub Actions have
not run. No XPI has been generated or installed and no production
Zotero/Notion data has been accessed.
...
| R-M02 unsupported runtime claim    | The manifest allowed Zotero 10 before current production-adapter runtime evidence existed.                                                          | Limit the first isolated scope to Zotero 9; retain Zotero 10 code contracts; require the actual production adapter/store smoke before push authorization.                              | runtime manifest test, smoke source                        |
...
The supplied isolated Zotero 9.0.6 primitive transaction spike is validated
with `overall: PASS`, but it is not current production-adapter/store evidence.
The repository production smoke remains **PENDING USER RUN**. Zotero 10 real
runtime validation also remains pending and must not be inferred from
capability-contract tests.

The implementation is not release-ready. A structured PASS from the isolated
Zotero 9.0.6 production-adapter smoke is required before push authorization is
even requested; independent review and later isolated plugin/Notion validation
remain separate gates. No production installation or production-data E2E is
